...
export class ConditionVariable {
  private waitQueue: ConditionQueuedTask[] = []
  private monitor: MonitorMixin
  ...
  async wait(timeout?: number): Promise<boolean> {
    const state = this.monitor._exitForCond()
    ...
    try {
      ...
      await this.monitor._enterForCond(state)
    }
  }
  ...
  async waitWhile(condition: () => boolean, timeout?: number): Promise<boolean> {
    ...
    while (condition()) {
      ...
      const signaled = await this.wait(remainingTimeout)
      if (!signaled) {
        return false // Timed out during wait
      }
    }

    return true // Condition became false
  }
  ...
  signal(): void {
    ...
    if (task) {
      // Use setImmediate to ensure the waiting task runs in next tick
      setImmediate(() => task.resolve())
    }
  }
   ...
   */
  broadcast(): void {
    const tasks = this.waitQueue.splice(0)
    tasks.forEach((task) => {
      setImmediate(() => task.resolve())
    })
  }
...
export class MonitorMixin {
  ...
  private lockQueue: QueuedTask[] = []
  private condQueue: Array<{ resolve: () => void; reject: (error: Error) => void; state: LockOwner }> = []
  ...
  static extendObject<T extends object>(obj: T): T & MonitorMixin {
    const monitor = new MonitorMixin()
    const extended = obj as T & MonitorMixin

    // Copy all monitor methods to the object with proper binding
    // Using Object.defineProperty to avoid type errors
    Object.defineProperty(extended, "enter", {
      value: monitor.enter.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "exit", {
      value: monitor.exit.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "tryEnter", {
      value: monitor.tryEnter.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "synchronize", {
      value: monitor.synchronize.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "newCond", {
      value: monitor.newCond.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "monLocked", {
      value: monitor.monLocked.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "monOwned", {
      value: monitor.monOwned.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "monEnter", {
      value: monitor.monEnter.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "monExit", {
      value: monitor.monExit.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "monTryEnter", {
      value: monitor.monTryEnter.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "tryMonEnter", {
      value: monitor.tryMonEnter.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "monSynchronize", {
      value: monitor.monSynchronize.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "_exitForCond", {
      value: monitor._exitForCond.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "_enterForCond", {
      value: monitor._enterForCond.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
    Object.defineProperty(extended, "_wakeupNext", {
      value: (monitor as unknown as { _wakeupNext: () => void })._wakeupNext.bind(monitor),
      writable: true,
      enumerable: false,
      configurable: true,
    })
  ...
  async enter(): Promise<LockOwner> {
    ...
    const newOwner: LockOwner = { id: ownerId, count: 1 }
  ...
  exit(owner: LockOwner): void {
    if (this.owner === null || this.owner.id !== owner.id) {
    ...
    if (this.owner.count === 0) {
      ...
      // Wake up next waiter (prioritize condition reacquisition)
      this._wakeupNext()
    }
  }
   ...
   */
  async synchronize<T>(block: (owner: LockOwner, monitor: this) => T | Promise<T>): Promise<T> {
    const owner = await this.enter()
    try {
      return await block(owner, this)
    } finally {
      this.exit(owner)
    }
  }
  ...
  monOwned(owner?: LockOwner): boolean {
    ...
    return this.owner !== null && this.owner.id === owner.id
  }
   ...
   */
  async monSynchronize<T>(block: (owner: LockOwner, monitor: this) => T | Promise<T>): Promise<T> {
    return this.synchronize(block)
  }
  ...
  _exitForCond(): LockOwner {
    ...
    // Wake up next waiter (either normal or condition reacquire)
    this._wakeupNext()
   ...
   */
  async _enterForCond(state: LockOwner): Promise<void> {
    // Register for condition reacquisition (priority queue)
    if (this.locked) {
      await new Promise<void>((resolve, reject) => {
        this.condQueue.push({ resolve, reject, state })
      })
      return // State already restored by _wakeupNext
    }

    // Lock is available immediately
    this.locked = true
    this.owner = state
  }
   ...
   * @private
   */
  private _wakeupNext(): void {
    // First check if there are any condition variable reacquisitions waiting
    const condTask = this.condQueue.shift()
    if (condTask) {
      this.locked = true
      this.owner = condTask.state
      setImmediate(() => condTask.resolve())
      return
    }

    // Otherwise wake up a normal waiter
    const task = this.lockQueue.shift()
    if (task) {
      setImmediate(() => task.resolve())
    }
  }
 ...
 */
export class Monitor extends MonitorMixin {
  /**
  ...
  async synchronize<T>(block: (owner: LockOwner, monitor: this) => T | Promise<T>): Promise<T>
  async synchronize<T>(block: (() => T | Promise<T>) | ((owner: LockOwner, monitor: this) => T | Promise<T>)): Promise<T> {
    if (block.length === 0) {
      // Block doesn't take parameters - use simple wrapper
      return super.synchronize(async () => await (block as () => T | Promise<T>)())
    } else {
      // Block takes parameters - pass them through
      return super.synchronize(block as (owner: LockOwner, monitor: this) => T | Promise<T>)
    }
  }
}
